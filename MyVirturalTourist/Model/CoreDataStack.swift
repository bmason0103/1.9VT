...
//    }
//
//    static func shared2() -> DataController {
//        struct Singleton {
...
struct CoreDataStack {
    ...
    static func shared() -> CoreDataStack {
        struct Singleton {
            static var shared = CoreDataStack(modelName: "MyVirtualTourist")!
        }
        return Singleton.shared
    }
    ...
    func fetchPin(_ predicate: NSPredicate, entityName: String, sorting: NSSortDescriptor? = nil) throws -> ThePin? {
        ...
        guard let pin = (try context.fetch(fr) as! [ThePin]).first else {
            return nil
        }
        return pin
    }
    ...
    func fetchAllPins(_ predicate: NSPredicate? = nil, entityName: String, sorting: NSSortDescriptor? = nil) throws -> [ThePin]? {
        ...
        guard let pin = try context.fetch(fr) as? [ThePin] else {
            return nil
        }
        return pin
    }
    ...
    func fetchPhotos(_ predicate: NSPredicate? = nil, entityName: String, sorting: NSSortDescriptor? = nil) throws -> [Photos]? {
        ...
        guard let photos = try context.fetch(fr) as? [Photos] else {
            return nil
        }
        return photos
    }
